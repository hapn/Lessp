...
final class Lunar {
	...
	*/ 
	function convertSolarToLunar($year, $month, $day)
	{
		...
		return $this->getLunarByBetween($year, $this->getDaysBetweenSolar($year, $month, $day, $yearData[1], $yearData[2]));
	}
	...
	function convertSolarMonthToLunar($year, $month){
		...
		$dd = date('t', mktime(0, 0, 0, $month, 1, $year));

		$lunar_ary = array();
		$special = ($year == $this->MIN_YEAR && $month <= 2);
		for ($i = 1; $i <= $dd; $i++){
			if ($special && $i <= 9) {
				$lunar_ary[$i] = array(1891, '正月', '初一', '辛卯', 1, 1, '兔');
				continue;
			}
	...
	* @param day 
	*/ 
	function getDaysBetweenLunar($year,$month,$day)
	{ 
	...
	* @param dday 阴历初一对应的阳历天数 
	*/ 
	function getDaysBetweenSolar($year, $cmonth, $cday, $dmonth, $dday)
	{ 
		$a = mktime(0, 0, 0, $cmonth, $cday, $year);
		$b = mktime(0, 0, 0, $dmonth, $dday, $year);
