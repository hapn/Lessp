...
final class Db
{
	...
	static function init ( $conf )
	{
		...
		DbContext::$dbalias = isset($conf['db_alias']) ? $conf['db_alias'] : array();
		
		DbContext::$textDB = empty($conf['text_db']) ? 'text' : $conf['text_db'];
		DbContext::$textTable = empty($conf['text_db']) ? 'text' : $conf['text_table'];
	...
	static function get ( $db_name = null )
	{
		...
		// 检查是否有别名
		if ( isset(DbContext::$dbalias[$db_name]) ) {
			$real_name = DbContext::$dbalias[$db_name];
		} else {
			$real_name = $db_name;
		}
	 ...
	 * @param int $count 分配id的个数。如果count>1，则会返回最后一个id
	 */
	static function newGUID ( $name, $count = 1 )
	{
		...
		$changeRows = $db->queryBySql(
				'UPDATE ' . DbContext::$guidDB . '.' . DbContext::$guidTable .
						 ' set guid_value = LAST_INSERT_ID(guid_value+?) where guid_name = ?', $count, $name);
		if ( ! $changeRows ) {
	 ...
	 * 向文本库中提交一批文本数据
	 * 
	 * @param mixed $textdata  可以是单个string或者string数组
	 ...
	 * 查询文本数据.
	 * 
	 * @param string|array $textId 要查询的文本id
	...
	static function queryText ( $textId )
	{
		...
		foreach ( $arrTable as $table => $batchid ) {
			...
			if ( ! empty($batchid) ) {
				$_arr = $db->table($table)
					->field('text_id', 'text_content', 'text_zip_flag')
					->in('text_id', $batchid)
					->get();
